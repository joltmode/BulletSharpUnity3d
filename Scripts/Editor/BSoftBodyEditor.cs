  ...
  public class BSoftBodyEditor : Editor
  {
   ...
   "Default: Cluster soft body self collision.";
    ...
    public override void OnInspectorGUI()
    {
      ...
      if (target is BSoftBodyWMesh)
      {
        BSoftBodyWMesh sb = (BSoftBodyWMesh)target;

        if (EditorHelpers.InspectorButton("Bind Nodes To Anchors", 300, 15, GUIBlue))
        {
          sb.BindNodesToAnchors();
        }
        EditorGUILayout.HelpBox(sb.DescribeAnchors(), MessageType.Info);
